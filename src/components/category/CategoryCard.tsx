...
export function CategoryCard({ category, variant = 'default' }: CategoryCardProps) {
    ...
    const cardClasses = {
        ...
        large: 'group hover:shadow-2xl transition-all duration-300 overflow-hidden cursor-pointer border-2 hover:border-primary/50',
    ...
    return (
        <Link href={category.href} className="block h-full">
            ...
            >
                <div className={`relative ${sizeClasses[variant]} overflow-hidden ${variant === 'default' ? 'bg-muted/30' : 'bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 dark:from-emerald-950 dark:via-teal-950 dark:to-cyan-950'}`}>
                ...
                <CardContent className={`${variant === 'large' ? 'p-6' : variant === 'default' ? 'p-3' : 'p-4'} relative`}>
                    {variant === 'default' ? (
                        // Minimal centered layout for default variant
                        ...
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex-1">
                                <h3 className={`${variant === 'large' ? 'text-xl' : 'text-base'} font-bold mb-1 group-hover:text-primary transition-colors duration-300`}>
                                    {category.name}
                                </h3>
